...
# json provide all items of a sport
@app.route('/publiccatalog/<int:category_id>/items/json')
def sportcatItemsJSON(category_id):
    cat_list = session.query(Categories).filter_by(id=category_id).one()
    items = session.query(SportItem).filter_by(
        sportCategories_id=category_id).all()
    return jsonify(SportItems=[i.serialize for i in items])


# json provide all sports
@app.route('/publiccatalog/json')
def categoriesJSON():
    cat_list = session.query(Categories).all()
    return jsonify(name=[r.serialize for r in cat_list])


# json provide single Item
@app.route('/publiccatalog/<int:sportItem_id>/item/json')
def itemJSON(sportItem_id):
    Item = session.query(SportItem).filter_by(id=sportItem_id).one()
...
def showCatalog():
    ...
    latest_items = session.query(SportItem).order_by(asc(SportItem.id)) \
        .limit(10)
    return render_template('sportcategories.html',
                           sports=sports, latest_items=latest_items)
...
def selectCategory(category_id):
    ...
    sport_items = session.query(SportItem)\
        .filter_by(sportCategories_id=category_id).all()
    return render_template('categoryItemList.html',
                           sports=sports, sport_items=sport_items,
                           sports1=sports1)


@app.route('/publiccatalog/items/', methods=['GET', 'POST'])
def showCategory():
    if 'username' not in login_session:
        return redirect('/publiccatalog/')
    sports = session.query(Categories).order_by(asc(Categories.name))
    latest_items = session.query(SportItem).order_by(asc(SportItem.id)) \
        .limit(10)
    return render_template('loginsportcategories.html', sports=sports,
...
def editItem(sportItem_id):
    ...
    if creator.id != login_session['user_id']:
        return render_template('noncreator.html',
    ...
    if request.method == 'POST':
        ...
        return render_template('editItemCatalog.html',
                               sportItem_id=sportItem_id, creator=creator)
...
def deletedItem(sportItem_id):
        ...
        if request.method == 'POST':
            ...
            return render_template('deleteItemName.html',
                                   sportItem_id=sportItem_id,
